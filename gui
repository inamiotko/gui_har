...
def browse_files():
    ...
    def chosen_model():
        ...
        if chosen == 'First model':
            loaded_model = keras.models.load_model('/Users/iganamiotko/Desktop/trackers/model_1.h5')
        else:
            loaded_model = keras.models.load_model('/Users/iganamiotko/Desktop/trackers/model_2.h5')
...
def browse_files_graph():
    ...
    sns.set()
    graphs_chooser.place(x=520, y=300)
    ...
    def callback(event):
        if filename.endswith('.json') and graphs_chooser.get() == 'amount of steps throughout the time period':
            ...
            sns.heatmap(data3, cbar_kws={'label': 'Intensity'})
            ax.invert_yaxis()
...
label_results = tk.Label(window, text="Recognised human activity:  ", width=35, height=7, fg="black")

label_files.place(x=20, y=50)
label_files_img.place(x=20, y=300)
button1.place(x=390, y=50)
button3.place(x=390, y=300)
chooser.place(x=520, y=55)
